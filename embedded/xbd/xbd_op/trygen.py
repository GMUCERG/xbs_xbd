...
if tunebytes:
  print '#define SMALL' # Microcontrollers are too small to use large version
...
for v,n in var:
  if n == 0:
    print 'static unsigned long long %slen;' % v
  else:
...
print ''
print 'void test_allocate(unsigned char *buf)'
print '{'
pv = None
pn = None
pprefix = None
def print_alloc_line(second):
  global pv
  global pn
  global pprefix
  s = ''
  ...
  for v,n in var:
    ...
    if pv == None:
      print '  %s%s = buf+CANARY_SZ;' % (v, s)
    else:
      print '  %s%s = %s+%s%s+CANARY_SZ*2;' % (v, s, pv, pprefix, pn)
    pv = v
    pn = n
    pprefix = prefix

print_alloc_line(False)
print_alloc_line(True)


print '  //Randomize contents'
print '  for (size_t i = 0; i < MAX_TESTBUFFER_BYTES; i++){'
print '      buf[i]=(unsigned char)(myrandom() & 0xFF);'
print '  }'
print '}'
...
print '{'
indent = '  '
# loop counters are simply unsigned integers, long long is overkill for
# microcontroller
if operation in ['stream','secretbox','box']:
  print indent+'unsigned j;'
print indent+'unsigned loop;'
print indent
print indent+'if(!setjmp(test_fail_jmp)){'
indent += '  '
...
print indent+'}else{'
indent += '  '
print indent+'return FAIL_CHECKSUM;'
indent = indent[:-2]
print indent+'}'
print '}'
