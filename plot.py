...
def main():
    ...
    for bx in  s.query(xbxr.BuildExec).order_by(xbxr.BuildExec.id.desc()):

        impl = bx.build.implementation.path

        # q is filtered for everything in last runsession, only encryption results
        q = (s.query(xbxro.CryptoAeadRun)
        .join(xbxr.BuildExec)
        .join(xbxr.RunSession)
        .filter(xbxr.BuildExec.id == bx.id)
        .filter(xbxro.CryptoAeadRun.mode == 'enc'))
        ...
        plt.suptitle(bx.build.implementation.path)
        ax1 = plt.subplot(311)
        ...
        #ax3.axis([0, 2000, 1, 10000])
        plt.suptitle(impl)
        plt.savefig("plots/"+str(impl).replace('/','_')+"_plot.pdf")
