...
void i2cInit(uint8_t addr){
    ...
    MAP_I2CSlaveInit(I2C0_BASE, addr);
...
void i2cHandle(void){
    ...
    while(true){
        status = MAP_I2CSlaveStatus(I2C0_BASE);

        switch(state){
            case STATE_START:
                if(MAP_I2CSlaveIntStatusEx(I2C0_BASE, false) & I2C_SLAVE_INT_START){
                    rx_idx = 0;
                    tx_idx = 0;
                    invalid = false;
                    if(status == I2C_SLAVE_ACT_RREQ_FBR){
                        MAP_I2CSlaveIntClearEx(I2C0_BASE, I2C_SLAVE_INT_START);
                        state = STATE_RX;
            ...
            case STATE_RX:
                if(status == I2C_SLAVE_ACT_RREQ ||
                    ...
                    I2cBuff[rx_idx++] = MAP_I2CSlaveDataGet(I2C0_BASE);
                }
                if(MAP_I2CSlaveIntStatusEx(I2C0_BASE, false) & I2C_SLAVE_INT_STOP){
                    MAP_I2CSlaveIntClearEx(I2C0_BASE, I2C_SLAVE_INT_STOP);
                    state = STATE_START;
                    if(!invalid){
            ...
            case STATE_TX:
                if(status == I2C_SLAVE_ACT_TREQ){
                    ...
                    MAP_I2CSlaveDataPut(I2C0_BASE, I2cBuff[tx_idx++]);
                }
                if(MAP_I2CSlaveIntStatusEx(I2C0_BASE, false) & I2C_SLAVE_INT_STOP){
                    MAP_I2CSlaveIntClearEx(I2C0_BASE, I2C_SLAVE_INT_STOP);
                    state = STATE_START;
